...
data:
  ...
  attributes:
    links:
    - https://maspypy.github.io/library/poly/online/online_convolution.hpp
  bundledCode: "#line 2 \"Convolution/OnlineConvolution.hpp\"\n\n// source: https://maspypy.github.io/library/poly/online/online_convolution.hpp\n\
  ...
  code: "#pragma once\n\n// source: https://maspypy.github.io/library/poly/online/online_convolution.hpp\n\
    ...
    \         b1 = g, b1.resize(2 * w);\n            NTT<T>::ntt(b1);\n          \
    \  fm.resize(w << 1), gm.resize(w << 1);\n            for (int i = 0; i < w; ++i)\
    \ fm[i + w] = b0[i * 2], gm[i + w] = b1[i * 2];\n            for (int i = 0; i\
    \ < 2 * w; ++i) b0[i] *= b1[i];\n            s = w - 2;\n            h.resize(2\
    \ * s + 2);\n        }\n        else {\n            b0.assign(f.end() - w, f.end()),\
    \ b0.resize(2 * w);\n            NTT<T>::ntt(b0);\n            for (int i = 0;\
    \ i < 2 * w; ++i) b0[i] *= gm[i + (w << 1)];\n            b1.assign(g.end() -\
    \ w, g.end()), b1.resize(2 * w);\n            NTT<T>::ntt(b1);\n            for\
    \ (int i = 0; i < 2 * w; ++i) b0[i] += b1[i] * fm[i + (w << 1)];\n           \
    \ s = w - 1;\n        }\n        NTT<T>::ntt<true>(b0);\n        for (int i =\
    \ 0; i <= s; ++i) h[p + i] += b0[s + i];\n        return h[p++];\n    }\n};\n"
  dependsOn:
  - Polynomial/NTT.hpp
  - Numeric/Modint.hpp
...
documentation_of: Convolution/OnlineConvolution.hpp
layout: document
redirect_from:
- /library/Convolution/OnlineConvolution.hpp
- /library/Convolution/OnlineConvolution.hpp.html
