...
data:
  ...
  bundledCode: "#line 1 \"test/2_aoj/minimum_enclosing_circle.test.cpp\"\n#define\
    ...
    \ - q, b - q));\n    return Geometry<MulT, eps>::sign(det);\n}\n#line 6 \"Geometry/minimum_enclosing_circle.hpp\"\
    \n\ntemplate<typename Point, typename T = DefaultFloat<typename Point::value_type>,\
    \ T eps = std::is_same_v<typename Point::value_type, T> ? Point::eps_val : get_default_eps<T>(),\
...
layout: document
redirect_from:
- /verify/test/2_aoj/minimum_enclosing_circle.test.cpp
- /verify/test/2_aoj/minimum_enclosing_circle.test.cpp.html
