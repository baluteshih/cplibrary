...
data:
  ...
  bundledCode: "#line 1 \"test/2_aoj/polygon-point_containment.test.cpp\"\n#define\
    \ PROBLEM \"https://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=CGL_3_C\"\
    ...
    }\n#line 3 \"test/2_aoj/polygon-point_containment.test.cpp\"\n\n#line 2 \"Geometry/polygon.hpp\"\
    ...
    \ std::vector<Pt<T, eps, MulT>>, public Geometry<MulT, eps> {\npublic:\n    using\
    \ Point = Pt<T, eps, MulT>;\n    using Line = Ln<T, eps, MulT>;\n    using std::vector<Pt<T,\
    \ eps, MulT>>::vector;\n    using Geometry<MulT, eps>::sign;\n    using Geometry<MulT,\
    \ eps>::cmp;\n    Polygon(const std::vector<Point> &vec) : Polygon(vec.begin(),\
    \ vec.end()) {} \n    template <typename U, U _eps, typename _MulT>\n    Polygon(const\
    \ Polygon<U, _eps, _MulT>& other) : Polygon(other.begin(), other.end()) {}\n \
    \   Point& at(int index) {\n        index %= this->size();\n        if (index\
    \ < 0) index += this->size();\n        return (*this)[index];\n    }\n    const\
    ...
    \    }\n    int polygon_dir() {\n        return sign(area());\n    }\n    friend\
    \ bool pointInPoly(const Point &p, const Polygon &a, bool strict = false) {\n\
    \        int n = a.size(), res = 0;\n        for (int i = 0; i < n; i++) {\n \
    \           Point u = a[i], v = a[(i + 1) % n];\n            if (pointOnSeg(p,\
    \ Ln(u, v))) { return !strict; }\n            if (cmp(u.y, v.y) <= 0) { std::swap(u,\
    ...
    \ - A, B - A), 1);\n                            segs.emplace_back(rat(D - A, B\
    \ - A), -1);\n                        }\n                    }\n             \
    \   }\n                std::ranges::sort(segs);\n                for (auto &s\
    \ : segs) s.first = std::clamp(s.first, Ret(0), Ret(1));\n                Ret\
    \ sum = 0;\n                int cnt = segs[0].second;\n                for (int\
    \ j = 1; j < int(segs.size()); ++j) {\n                    if (!cnt) sum += segs[j].first\
    \ - segs[j - 1].first;\n                    cnt += segs[j].second;\n         \
    \       }\n                res += Ret(cross(A, B)) * sum;\n            }\n   \
    \     return res / 2;\n    }\n};\n\ntemplate <typename PointType>\nstruct PolygonType;\n\
    \ntemplate <typename T, T eps, typename MulT>\nstruct PolygonType<Pt<T, eps, MulT>>\
    \ {\n    using type = Polygon<T, eps, MulT>;\n};\n#line 5 \"test/2_aoj/polygon-point_containment.test.cpp\"\
    \n\nusing Point = Pt<int>;\nusing polygon = Polygon<int>;\n\nint main() {\n  \
    \  ios::sync_with_stdio(0), cin.tie(0);\n    int n;\n    cin >> n;\n    polygon\
    \ poly(n);\n    for (auto &p : poly)\n        cin >> p;\n    int q;\n    cin >>\
    \ q;\n    while (q--) {\n        Point p;\n        cin >> p;\n        if (pointStrictlyInPoly(p,\
    \ poly)) cout << \"2\\n\";\n        else if (pointInPoly(p, poly)) cout << \"\
    1\\n\";\n        else cout << \"0\\n\";\n    }\n}\n"
  ...
  verifiedWith: []
documentation_of: test/2_aoj/polygon-point_containment.test.cpp
layout: document
redirect_from:
- /verify/test/2_aoj/polygon-point_containment.test.cpp
- /verify/test/2_aoj/polygon-point_containment.test.cpp.html
