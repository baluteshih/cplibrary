...
data:
  ...
  bundledCode: "#line 1 \"test/4_codeforces/101242J.test.cpp\"\n#define PROBLEM \"\
    ...
    \ one.emplace(a, b);\n    }\n    auto convex = convexHull(std::vector<Point>(one.begin(),\
    ...
    \       assert(side(p, a, b) >= 0);\n        {\n            int r = std::ranges::lower_bound(vec,\
    \ b - p, cmp) - vec.begin();\n            int l = std::ranges::lower_bound(vec,\
    \ a - p, cmp) - vec.begin();\n            if (l <= r) add_itv(l, r);\n       \
    \     else add_itv(0, r), add_itv(l, int(vec.size()) - 1), flag = 1;\n       \
    \ }\n        {\n            int r = std::ranges::lower_bound(vec, p - b, cmp)\
    \ - vec.begin();\n            int l = std::ranges::lower_bound(vec, p - a, cmp)\
    \ - vec.begin();\n            if (l <= r) add_itv(l, r);\n            else add_itv(0,\
    \ r), add_itv(l, int(vec.size()) - 1), flag = 1;\n        }\n    }\n    if (int(itv.size())\
    \ > 1 && flag) {\n        std::ranges::sort(itv);\n        int cur = 0;\n    \
