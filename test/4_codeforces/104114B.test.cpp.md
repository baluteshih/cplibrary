...
data:
  ...
  bundledCode: "#line 1 \"test/4_codeforces/104114B.test.cpp\"\n#define PROBLEM \"\
    ...
    \ vals.end());\n}\n#line 4 \"test/4_codeforces/104114B.test.cpp\"\n\n#line 2 \"\
    ...
    \ l]\n#line 6 \"test/4_codeforces/104114B.test.cpp\"\n\nusing Point = Pt<ll>;\n\
    ...
    \ = tangentLineToConvex(arr[i], convex);\n        assert(side(arr[i], convex[r],\
    \ convex[l]) >= 0);\n        Point lft = arr[i] - convex[l];\n        Point rgt\
    \ = convex[r] - arr[i];\n        if (betweenAngle(Point(0, 0), lft, rgt, Point(1,\
    \ 0), 1))\n            ++cur;\n        else if (rgt.x > 0 && rgt.y == 0)\n   \
    \         ++cur;\n        event.push_back(Event{lft, 1});\n        event.push_back(Event{rgt,\
    \ -1});\n    }\n    ranges::sort(event, [&](auto &a, auto &b) {\n        return\
    \ polar(a.vec, b.vec);\n    });\n    int ans = cur;\n    for (int i = 0, j = 0;\
    \ i < SZ(event); i = j) {\n        while (j < SZ(event) && sameDirection(event[i].vec,\
    \ event[j].vec))\n            cur += event[j].v, ++j;\n        ans = max(ans,\
    \ cur);\n    }\n    cout << ans << \"\\n\";\n}\n"
  code: "#define PROBLEM \"https://codeforces.com/gym/104114/problem/B\"\n#define\
  ...
  verifiedWith: []
documentation_of: test/4_codeforces/104114B.test.cpp
layout: document
redirect_from:
- /verify/test/4_codeforces/104114B.test.cpp
- /verify/test/4_codeforces/104114B.test.cpp.html
