...
data:
  ...
  bundledCode: "#line 1 \"test/1_library_checker/tree/lca.test.cpp\"\n#define PROBLEM\
    ...
    Graph/base.hpp\"\n\ntemplate<typename W>\nstruct edge_data {\n    int from, to;\n\
    \    W weight;\n    edge_data() = default;\n    edge_data(int u, int v, const\
    \ W &w) : from(u), to(v), weight(w) {}\n};\ntemplate<>\nstruct edge_data<void>\
    \ { \n    int from, to; \n    edge_data() = default;\n    edge_data(int u, int\
...
layout: document
redirect_from:
- /verify/test/1_library_checker/tree/lca.test.cpp
- /verify/test/1_library_checker/tree/lca.test.cpp.html
