...
data:
  ...
  bundledCode: "#line 1 \"test/1_library_checker/geometry/furthest_pair.test.cpp\"\
    \n#define PROBLEM \"https://judge.yosupo.jp/problem/furthest_pair\"\n#line 2 \"\
    assumption.hpp\"\n\n#include <cassert>\n#include <bits/stdc++.h>\n#line 3 \"test/1_library_checker/geometry/furthest_pair.test.cpp\"\
    ...
    \ rotating_calipers(int(convex.size()), [&](int o, int a, int b) {\n        return\
    \ Point::cmp(cross(convex[o], convex[(o + 1) % int(convex.size())], convex[a]),\
    \ cross(convex[o], convex[(o + 1) % int(convex.size())], convex[b])) > 0; \n \
    \   });\n    std::pair<int, int> ans = std::make_pair(0, res[0]);\n    for (int\
    \ i = 1; i < int(convex.size()); ++i)\n        if (dist2(convex[ans.first], convex[ans.second])\
    \ < dist2(convex[i], convex[res[i]]))\n            ans = std::make_pair(i, res[i]);\n\
    \    return std::make_pair(std::ranges::find(dots, convex[ans.first]) - dots.begin(),\
    \ std::ranges::find(dots, convex[ans.second]) - dots.begin());\n}\n#line 5 \"\
  ...
  verifiedWith: []
documentation_of: test/1_library_checker/geometry/furthest_pair.test.cpp
layout: document
redirect_from:
- /verify/test/1_library_checker/geometry/furthest_pair.test.cpp
- /verify/test/1_library_checker/geometry/furthest_pair.test.cpp.html
