...
data:
  ...
  bundledCode: "#line 1 \"test/1_library_checker/geometry/furthest_pair.test.cpp\"\
    ...
    \ res[0]);\n    for (int i = 1; i < SZ(convex); ++i)\n        if (dist2(convex[ans.first],\
    \ convex[ans.second]) < dist2(convex[i], convex[res[i]]))\n            ans = std::make_pair(i,\
    \ res[i]);\n    return std::make_pair(ranges::find(dots, convex[ans.first]) -\
    \ dots.begin(), ranges::find(dots, convex[ans.second]) - dots.begin());\n}\n#line\
    \ 5 \"test/1_library_checker/geometry/furthest_pair.test.cpp\"\n\nusing Point\
    \ = Pt<ll>;\n\nvoid solve() {\n    int n;\n    cin >> n;\n    vector<Point> arr(n);\n\
