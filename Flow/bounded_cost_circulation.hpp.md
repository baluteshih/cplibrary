...
data:
  ...
  attributes:
    links: []
  bundledCode: "#line 2 \"Flow/bounded_cost_circulation.hpp\"\n\n#line 2 \"Flow/min_cost_circulation.hpp\"\
    ...
    \ (v != -1)\n                std::swap(x ^= 1, fae[v]), std::swap(u, fa[v]), std::swap(u,\
    \ v);\n        }\n        else {\n            int z = u, dir = 0;\n          \
    ...
    \ x ^= 1, std::swap(u, v);\n            while (u != z)\n                std::swap(x\
    \ ^= 1, fae[v]), std::swap(u, fa[v]), std::swap(u, v);\n        }\n    }\n   \
    \ void dfs(int u) {\n        vis[u] = visc;\n        for (auto [v, eid] : this->G[u])\n\
    \            if (vis[v] != visc && this->edges[eid].weight.flow)\n           \
    \     fa[v] = u, fae[v] = eid, dfs(v);\n    }\n    void solve() {\n        fa.assign(this->n(),\
    \ -1), fae.assign(this->n(), -1);\n        ++visc, dfs(0);\n        for (int fail\
    \ = 0; fail < int(this->edges.size());)\n            for (int i = 0; i < int(this->edges.size());\
    \ ++i)\n                if (this->edges[i].weight.flow && this->edges[i].weight.cost\
    \ < phi(this->edges[i].from) - phi(this->edges[i].to))\n                    fail\
    \ = 0, pushflow(i), ++visc;\n                else\n                    ++fail;\n\
    \    }\n    void add_edge(int a, int b, T cap, C cost) {\n        super::add_edge(a,\
    \ b, CostFlowWeight<T, C>(cap, cost, cap));\n        super::add_edge(b, a, CostFlowWeight<T,\
    \ C>(T(0), -cost, T(0)));\n    }\n    C get_cost() {\n        C res = 0;\n   \
    \     for (int i = 0; i < int(this->edges.size()); i += 2)\n            res +=\
    \ this->edges[i].weight.cost * (this->edges[i].weight.cap - this->edges[i].weight.flow);\n\
    ...
    \             if (!inq[v]) {\n                        inq[v] = true;\n       \
    \                 q.push(v);\n                    }\n                }\n     \
    \   }\n        return d;\n    }\n};\n#line 4 \"Flow/bounded_cost_circulation.hpp\"\
    \n\ntemplate<typename T, typename C = T>\nclass bounded_cost_circulation : public\
    \ min_cost_circulation<T, C> { // 0-base\npublic:\n    using super = min_cost_circulation<T,\
    \ C>;\n    using super::add_edge;\n    int real_n;\n    std::vector<T> cnt;\n\
    \    bounded_cost_circulation(int _n) : super(_n + 2), real_n(_n), cnt(real_n\
    \ + 2) {}\n    void add_demand(int u, T d) {\n        cnt[u] += d;\n    }\n  \
    \  void add_edge(int u, int v, T lcap, T rcap, C cost) {\n        cnt[u] -= lcap,\
    \ cnt[v] += lcap;\n        super::super::add_edge(u, v, CostFlowWeight<T, C>(rcap,\
    \ cost, rcap - lcap));\n        super::super::add_edge(v, u, CostFlowWeight<T,\
    \ C>(T(0), -cost, T(0)));\n    }\n    bool solve(C inf) {\n        T sum = 0,\
    \ neg_sum = 0;\n        int added_cnt = 0;\n        for (int i = 0; i < real_n;\
    \ ++i)\n            if (cnt[i] > 0)\n                super::add_edge(real_n, i,\
    \ cnt[i], C(0)), sum += cnt[i], ++added_cnt;\n            else if (cnt[i] < 0)\n\
    \                super::add_edge(i, real_n + 1, -cnt[i], C(0)), neg_sum += cnt[i],\
    \ ++added_cnt;\n        super::add_edge(real_n + 1, real_n, std::numeric_limits<T>::max()\
    \ / 2, -inf), ++added_cnt;\n        super::solve();\n        bool res = (sum +\
    \ neg_sum == 0 && this->edges.back().weight.flow == sum);\n        while (added_cnt--)\
    \ this->pop_edge(), this->pop_edge();\n        return res;\n    }\n    bool solve(int\
    \ s, int t, C inf) {\n        super::add_edge(t, s, std::numeric_limits<T>::max()\
    \ / 2, -inf);\n        bool res = solve(inf);\n        this->pop_edge(), this->pop_edge();\n\
    \        return res;\n    }\n};\n"
  code: "#pragma once\n\n#include \"Flow/min_cost_circulation.hpp\"\n\ntemplate<typename\
    \ T, typename C = T>\nclass bounded_cost_circulation : public min_cost_circulation<T,\
    \ C> { // 0-base\npublic:\n    using super = min_cost_circulation<T, C>;\n   \
    \ using super::add_edge;\n    int real_n;\n    std::vector<T> cnt;\n    bounded_cost_circulation(int\
    \ _n) : super(_n + 2), real_n(_n), cnt(real_n + 2) {}\n    void add_demand(int\
    \ u, T d) {\n        cnt[u] += d;\n    }\n    void add_edge(int u, int v, T lcap,\
    \ T rcap, C cost) {\n        cnt[u] -= lcap, cnt[v] += lcap;\n        super::super::add_edge(u,\
    \ v, CostFlowWeight<T, C>(rcap, cost, rcap - lcap));\n        super::super::add_edge(v,\
    \ u, CostFlowWeight<T, C>(T(0), -cost, T(0)));\n    }\n    bool solve(C inf) {\n\
    \        T sum = 0, neg_sum = 0;\n        int added_cnt = 0;\n        for (int\
    \ i = 0; i < real_n; ++i)\n            if (cnt[i] > 0)\n                super::add_edge(real_n,\
    \ i, cnt[i], C(0)), sum += cnt[i], ++added_cnt;\n            else if (cnt[i] <\
    \ 0)\n                super::add_edge(i, real_n + 1, -cnt[i], C(0)), neg_sum +=\
    \ cnt[i], ++added_cnt;\n        super::add_edge(real_n + 1, real_n, std::numeric_limits<T>::max()\
    \ / 2, -inf), ++added_cnt;\n        super::solve();\n        bool res = (sum +\
    \ neg_sum == 0 && this->edges.back().weight.flow == sum);\n        while (added_cnt--)\
    \ this->pop_edge(), this->pop_edge();\n        return res;\n    }\n    bool solve(int\
...
documentation_of: Flow/bounded_cost_circulation.hpp
layout: document
redirect_from:
- /library/Flow/bounded_cost_circulation.hpp
- /library/Flow/bounded_cost_circulation.hpp.html
